...
class Client(fb303.FacebookService.Client, Iface):
  ...
  def set_token(self, token):
    ...
    Parameters:
     - token
    """
    self.send_set_token(token)
    return self.recv_set_token()

  def send_set_token(self, token):
    self._oprot.writeMessageBegin('set_token', TMessageType.CALL, self._seqid)
    args = set_token_args()
    args.token = token
    args.write(self._oprot)
    self._oprot.writeMessageEnd()
  ...
  def recv_set_token(self, ):
    ...
    if result.o1 is not None:
      raise result.o1
    raise TApplicationException(TApplicationException.MISSING_RESULT, "set_token failed: unknown result");
...
class Processor(fb303.FacebookService.Processor, Iface, TProcessor):
  def __init__(self, handler):
    ...
    self._processMap["set_token"] = Processor.process_set_token
  ...
  def process_set_token(self, seqid, iprot, oprot):
    args = set_token_args()
    args.read(iprot)
    iprot.readMessageEnd()
    result = set_token_result()
    try:
      result.success = self._handler.set_token(args.token)
    except MetaException as o1:
      result.o1 = o1
    oprot.writeMessageBegin("set_token", TMessageType.REPLY, seqid)
    result.write(oprot)
    oprot.writeMessageEnd()
    oprot.trans.flush()
...
class set_token_args:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('set_token_args')
...
class set_token_result:
  ...
  def write(self, oprot):
    if oprot.__class__ == TBinaryProtocol.TBinaryProtocolAccelerated and self.thrift_spec is not None and fastbinary is not None:
      oprot.trans.write(fastbinary.encode_binary(self, (self.__class__, self.thrift_spec)))
      return
    oprot.writeStructBegin('set_token_result')
    if self.success is not None:
      oprot.writeFieldBegin('success', TType.BOOL, 0)
      oprot.writeBool(self.success)
      oprot.writeFieldEnd()
